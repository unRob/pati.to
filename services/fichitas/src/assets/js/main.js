...
(function() {
  ...
  Config.prototype.on = function(key, cb) {
    this.callbacks[key] = this.callbacks[key] || [cb];
    return true;
  };

  Config.prototype.set = function(key, value) {
    var cb, evt_cbs, _i, _len;
    this.ds.setItem(key, value);
    evt_cbs = this.callbacks[key] || [];
    for (_i = 0, _len = evt_cbs.length; _i < _len; _i++) {
      cb = evt_cbs[_i];
      cb(value);
    }
    return value;
...
(function() {
  ...
  Game = function(config) {
    var id, p, playerCount, _i, _ref;
    this.players = {};
    $('#players').empty();
    playerCount = config.get('players.qty') || 2;
    console.log(playerCount);
    this.playerCount = parseInt(playerCount);
    for (id = _i = 1, _ref = this.playerCount; 1 <= _ref ? _i <= _ref : _i >= _ref; id = 1 <= _ref ? ++_i : --_i) {
      id = id;
      p = new Player(id);
      p.name = config.get("players.player." + id + ".name") || this.defaultName(id, this.playerCount);
      p.scores = JSON.parse(config.get("players.player." + id + ".scores")) || [];
      this.players[id] = p;
      $('#players').append(p.dom());
    }
    this.maxPoints = function() {
      return parseInt(config.get('game.maxPoints'), 10);
    };
    $('#players').attr('class', '').addClass("player-count-" + this.playerCount);
    window.refreshPlayersUI();
    this.started = config.get('game.started') || false;
    this.gameNo = config.get('game.number') || 0;
    this.gameNo = parseInt(this.gameNo);
    Game.instance = this;
    return this;
  };
  ...
  Game.prototype.addPoints = function(id, points) {
    var name, oid, other, player, verbo;
    if (!this.started) {
      this.started = true;
    ...
    if (player.score() >= this.maxPoints()) {
      console.log("lost", player.score(), this.maxPoints());
      this.started = false;
      name = "" + player.name + " ";
      verbo = 'perdió';
      if (player.name.match(/nosotros/i)) {
        verbo = 'perdimos';
      }
      ...
      return this.ended("" + name + verbo);
    }
    if (this.playerCount === 2) {
      oid = id === 1 ? 2 : 1;
      other = this.player(oid);
      other.addPoints(0);
      this.next();
    }
    return this;
  ...
  Game.prototype.ended = function(phrase) {
  ...
  Game.prototype.defaultName = function(id, count) {
    if (count === 2) {
      if (id === 1) {
        return "Nosotros";
      }
  ...
  Game.prototype.next = function() {
    var config, id, player, _ref;
    this.gameNo += 1;
    config = Config.instance;
    config.set('game.number', this.gameNo);
    _ref = this.players;
    for (id in _ref) {
      player = _ref[id];
  ...
  Game.prototype.name = function() {
    var str;
    if (str = Game.numbers[this.gameNo]) {
      return "" + str + " juego";
    } else {
      return "Juego " + (this.gameNo + 1);
    }
  };
  ...
  Player.prototype.addPoints = function(points) {
    return this.scores.push(points);
  };

  Player.prototype.score = function() {
    var s, score, str, _i, _len, _ref;
    s = 0;
    _ref = this.scores;
  ...
  Player.prototype.renderScore = function() {
    var _this = this;
    return function(nil, r) {
      return r(_this.score());
    };
  };

  Player.prototype.lost = function(qty) {};
...
(function() {
  $(function() {
    var changeName, config, finishScroll, guardaPlayer, isNumber, scrollFn, scrollTO, sumaPlayer;
    scrollTO = null;
    config = Config.instance;
    isNumber = function(n) {
      return !isNaN(parseInt(n)) && isFinite(n);
    };
    finishScroll = function(el) {
      var falta, fn, pc, sl, to, w;
    ...
    changeName = function(evt) {
      ...
      return el.text(newName);
    };
    sumaPlayer = function(evt) {
      var target, val;
      evt.preventDefault();
      ...
      return target.val('').focus();
    };
    guardaPlayer = function(evt) {
      var el, fichas, id, target, val;
      evt.preventDefault();
      target = $($(this).data('modifies'));
      ...
      if (fichas.length > 0) {
        fichas.each(function(i, ficha) {
          return val += parseInt(ficha.innerText, 10);
        });
        el.find('.suma').remove();
      }
      target.blur();
      el.addClass('active animating').animate({
        scrollLeft: 0
      }, 300, function() {
        return setTimeout(function() {
          return el.removeClass('active animating');
        }, 500);
      });
      target.val('');
      return window.addScore(id, val || 0);
    };
    window.refreshPlayersUI = function() {
      ...
      return $('.suma').on('click', 'li', function() {
        return $(this).remove();
      });
    };
    return $('#players').on('click', '.score', function(evt) {
      var el;
      evt.preventDefault();
      el = $(this).parents('.player');
      return el.addClass('active animating').animate({
        scrollLeft: el.width()
      }, 300, function() {
        return el.removeClass('active animating');
      });
...
(function() {
  ...
  $(function() {
    ...
    config.on('game.number', function() {
      return $('#game-name').text(game.name());
    });
    window.saveConfig = function(check) {
      if (check == null) {
        check = true;
      }
    ...
    window.addScore = function(player, points) {
      ...
      if (game.started) {
        return $("#player-" + player + " .score").text(game.player(player).score());
      }
    };
    ...
    $('#restart-game').on('click', function(evt) {
      evt.preventDefault();
